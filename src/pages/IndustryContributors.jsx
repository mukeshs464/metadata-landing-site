...
import { industrial_contributors } from "../constants";

const IndustryExpertDetails = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const contributors = industrial_contributors.filter(
    (expert) => expert.id === parseInt(id, 10)
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {contributors.map((contributor, index) => (
            ...
            >
              ...
              <p className="text-gray-600">{contributor.expertise || "N/A"}</p>
            </div>
          ))}
        </div>
      </div>
...
export default IndustryExpertDetails;
